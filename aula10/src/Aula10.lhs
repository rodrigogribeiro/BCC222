...
~~~~{.haskell}
foo :: [IO ()]
foo = map (\ n -> putStrLn ("Hello " ++ s)) ["João", "Maria"]
~~~~
...
- Modifique a implementação do jogo da forca para:
    - Imprimir a quantidade de erros
      que o jogador pode cometer. Idealmente,
      apresente essa informação como uma diagrama
      ASCII representando a figura de uma pessoa
      na forca.
    - A cada jogada, perguntar ao jogador se ele
      quer tentar adivinhar a palavra por completo.
